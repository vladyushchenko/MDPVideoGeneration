# Training configuration for out temporal mdp model
...
# CHECKPOINT=''
MODEL_NAME='MDPModel'
TRAINER_TYPE='temporal_gan'
MODEL_GENERATOR='VideoGenerator'
MODEL_DISCRIMINATORS='PatchImageDiscriminator TemporalVideoDiscriminator'
OPT_SUFFIX=''
...
OUT_FOLDER="$OUT_FOLDER_BASE"'/'"$DATASET_BASENAME""$SUFFIX_CAT"'/'"$MODEL_NAME""$SUFFIX_NOISE""$SUFFIX_DETER"/Sigma_"$TEMPORAL_SIGMA"_Beta_"$TEMPORAL_BETA"/Kernel_"$TCN_KERNEL_SIZE"_Blocks_"$TCN_BLOCKS"/Seed_"$SEED""$OPT_SUFFIX"/TrainLength_"$TRAINING_LENGTH"/Batch_"$BATCH_SIZE"
mkdir -p "$OUT_FOLDER"
